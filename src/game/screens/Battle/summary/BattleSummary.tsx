...
export const BattleSummary: React.FC<BattleSummaryProps> = ({
  ...
  const hasBattleLoot = !!rewards && (
    rewards.gold > 0 ||
    rewards.exp > 0 ||
    rewards.items.length > 0 ||
    rewards.spells.length > 0 ||
    rewards.weapons.length > 0 ||
    rewards.familiars.length > 0
  );
            ...
            {isVictory && rewardSelection ? (
                              ...
                              return (
                                ...
                                >
                                  <div className="reward-item-icon">
                                    {itemData.imagePath ? (
                                      <img src={itemData.imagePath} alt={getItemName(itemData)} />
                                    ) : (
                                      <span className="reward-item-emoji">{itemData.consumable ? '🧪' : '⚔️'}</span>
                                    )}
                                  </div>
                                  <div className="reward-item-name">{getItemName(itemData)}</div>
                                </div>
                              );
                            })}
                            {rewards.spells.map((spellId, idx) => {
                              const spell = getSpellById(spellId);
                              if (!spell) return null;

                              return (
                                <div key={`${spellId}-${idx}`} className="reward-item-card">
                                  <div className="reward-item-icon">
                                    <span className="reward-item-emoji">✨</span>
                                  </div>
                                  <div className="reward-item-name">{getSpellName(spell)}</div>
                                </div>
                              );
                            })}
                            {rewards.weapons.map((weaponId, idx) => {
                              const weapon = getWeaponById(weaponId);
                              if (!weapon) return null;

                              return (
                                <div key={`${weaponId}-${idx}`} className="reward-item-card">
                                  <div className="reward-item-icon">
                                    {weapon.imagePath ? (
                                      <img src={weapon.imagePath} alt={getWeaponName(weapon)} />
                                    ) : (
                                      <span className="reward-item-emoji">⚔️</span>
                                    )}
                                  </div>
                                  <div className="reward-item-name">{getWeaponName(weapon)}</div>
                                </div>
                              );
                            })}
                            {rewards.familiars.map((familiarId, idx) => {
                              const familiar = getFamiliarById(familiarId);
                              if (!familiar) return null;

                              return (
                                <div key={`${familiarId}-${idx}`} className="reward-item-card">
                                  <div className="reward-item-icon">
                                    <span className="reward-item-emoji">{familiar.icon}</span>
                                  </div>
                                  <div className="reward-item-name">{familiar.name}</div>
                                </div>
                              );
                            })}
                          </div>
              ...
              <>
                ...
                <div className="rewards-grid">
                  ...
                  {(rewards.items.length > 0 || rewards.spells.length > 0 || rewards.weapons.length > 0) && (
                    <div className="rewards-items-container">
                      <div className="rewards-items-label">Loot Earned:</div>
                      <div className="rewards-items-grid">
                        {rewards.items.map((item, idx) => {
                          const itemData = getItemById(item.itemId);
                          if (!itemData) return null;
