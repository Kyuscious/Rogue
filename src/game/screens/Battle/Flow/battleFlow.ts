...
import { getRandomLootByClass, ITEM_DATABASE } from '@data/items';
import { calculateExpReward } from '../../../entity/Player/experienceSystem';
import { getSpellById } from '@data/spells';
import { getWeaponById } from '@data/weapons';
import { getFamiliarById } from '@entities/Player/familiars';

export interface BattleReward {
  type: 'item' | 'gold' | 'health' | 'spell' | 'weapon' | 'familiar';
  itemId?: string;
  spellId?: string;
  weaponId?: string;
  familiarId?: string;
...
export function handleEnemyDefeat(
  ...
  // Add enemy-specific guaranteed/percentage drops for spells, weapons, and special items
  if (defeatedEnemy.lootDrops) {
    defeatedEnemy.lootDrops.items?.forEach(drop => {
      if (Math.random() <= drop.chance) {
        result.loot!.push({
          type: 'item',
          itemId: drop.itemId,
          amount: drop.quantity ?? 1,
        });
      }
    });

    defeatedEnemy.lootDrops.spells?.forEach(drop => {
      if (Math.random() <= drop.chance) {
        result.loot!.push({
          type: 'spell',
          spellId: drop.spellId,
        });
      }
    });

    defeatedEnemy.lootDrops.weapons?.forEach(drop => {
      if (Math.random() <= drop.chance) {
        result.loot!.push({
          type: 'weapon',
    ...
    defeatedEnemy.lootDrops.familiars?.forEach(drop => {
      if (Math.random() <= drop.chance) {
        result.loot!.push({
          type: 'familiar',
          familiarId: drop.familiarId,
        });
      }
    });
  }
  ...
  if (result.loot && result.loot.length > 0) {
    result.loot.forEach(reward => {
      if (reward.type === 'item' && reward.itemId) {
        const itemName = ITEM_DATABASE[reward.itemId]?.name || reward.itemId;
        victoryMessage += ` | 🎁 ${itemName}`;
      }
      if (reward.type === 'spell' && reward.spellId) {
        const spellName = getSpellById(reward.spellId)?.name || reward.spellId;
        victoryMessage += ` | ✨ ${spellName}`;
      }
      if (reward.type === 'weapon' && reward.weaponId) {
        const weaponName = getWeaponById(reward.weaponId)?.name || reward.weaponId;
        victoryMessage += ` | ⚔️ ${weaponName}`;
      }
      if (reward.type === 'familiar' && reward.familiarId) {
        const familiarName = getFamiliarById(reward.familiarId)?.name || reward.familiarId;
        victoryMessage += ` | 🐾 ${familiarName}`;
      }
...
export function applyVictoryRewards(
  ...
  if (result.loot) {
    result.loot.forEach(reward => {
      if (reward.type === 'item' && reward.itemId) {
        addInventoryItem({ itemId: reward.itemId, quantity: reward.amount || 1 });
      }
      if (reward.type === 'weapon' && reward.weaponId) {
        addWeapon(reward.weaponId);
      }
      if (reward.type === 'spell' && reward.spellId) {
        addSpell(reward.spellId);
      }
      if (reward.type === 'familiar' && reward.familiarId) {
        addFamiliar(reward.familiarId);
      }
      if (reward.type === 'health' && reward.amount) {
