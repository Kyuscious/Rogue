  ...
  if (isWeapon && weapon) {
    ...
    spell.effects.forEach(effect => {
      if (effect.type === 'damage' && effect.damageScaling) {
        ...
        if (effect.damageScaling.attackDamage) {
          effects.push(`${effect.damageScaling.attackDamage}% AD physical damage`);
        }
        if (effect.damageScaling.flatPhysicalDamage) {
          effects.push(`${effect.damageScaling.flatPhysicalDamage} flat physical damage`);
        }
        if (effect.damageScaling.trueDamage) {
          effects.push(`${effect.damageScaling.trueDamage} true damage`);
        }
        if (effect.damageScaling.missingHealthTrueDamage) {
          effects.push(`${effect.damageScaling.missingHealthTrueDamage}% target missing HP as true damage`);
        }
      }
