...
/**
 * Mark a familiar as discovered after it has joined a run
 */
export function discoverFamiliar(familiarId: string): void {
  const profile = getActiveProfile();
  const discoveredFamiliars = profile.stats.discoveredFamiliars || [];

  if (!discoveredFamiliars.includes(familiarId)) {
    updateProfile(profile.id, {
      stats: {
        ...profile.stats,
        discoveredFamiliars: [...discoveredFamiliars, familiarId],
        lastPlayedTimestamp: Date.now(),
      },
    });
  }
