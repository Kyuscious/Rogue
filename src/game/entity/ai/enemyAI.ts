...
function evaluateSpellUse(
  ...
  if (spell.effects) {
    spell.effects.forEach((effect: any) => {
      if (effect.type === 'damage' && effect.damageScaling) {
        // Calculate potential damage
        let potentialDamage = effect.damageScaling.flatPhysicalDamage || 0;
        if (effect.damageScaling.abilityPower) {
          potentialDamage += (enemyStats.abilityPower || 0) * (effect.damageScaling.abilityPower / 100);
        }
        if (effect.damageScaling.attackDamage) {
        ...
        if (effect.damageScaling.missingHealthTrueDamage) {
          const playerMissingHealth = Math.max(0, context.playerStats.health - context.player.hp);
          potentialDamage += playerMissingHealth * (effect.damageScaling.missingHealthTrueDamage / 100);
        }
...
export function decideEnemyAction(context: AIDecisionContext): AIAction {
  ...
  const currentActionType = context.currentActionType ?? 'attack';
  const canUseAttackAction = currentActionType === 'attack' || currentActionType === 'move';
  const canUseSpellAction = currentActionType === 'spell';
  ...
  // Evaluate only the actions allowed for this turn type
  const actions: Array<{ action: AIAction; score: number }> = [];
  
  if (canUseAttackAction) {
