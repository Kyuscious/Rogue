...
export function createShurimaSandSoldierSummon(ownerId: string, ownerLevel: number, summonIndex: number): Character {
  const baseSoldier = SHURIMA_MINIONS.find((enemy) => enemy.id === 'shurima_sand_soldier') || SHURIMA_MINIONS[0];

  return {
    ...baseSoldier,
    battleInstanceId: `${ownerId}_sand_soldier_${summonIndex}_${Date.now()}`,
    ownerId,
    isSummon: true,
    targetPriority: 0.85,
    canBeTargeted: true,
    level: ownerLevel,
    hp: baseSoldier.stats.health,
    stats: {
      ...baseSoldier.stats,
    },
    inventory: [],
