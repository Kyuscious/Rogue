...
2. [Adding New Familiars](#adding-new-familiars)
3. [Adding New Characters](#adding-new-characters)
4. [Adding New Abilities/Passives](#adding-new-abilitiespassives)
5. [Adding New Regions](#adding-new-regions)
...
Before adding new items, weapons, spells, or familiars:
- use the approved rarity palette and rarity-colored borders defined in [rarity-visual-guide.md](rarity-visual-guide.md)
...
- if you add a new weapon or spell `effects[].type`, make sure the Index presentation still exposes it cleanly; the grouping is data-driven, and optional icon polish can be added in the Index helpers
- keep names/descriptions spoiler-safe for undiscovered index entries
- for familiars, always define a clear automatic action cadence, active slot behavior, and acquisition source
...
- `src/game/familiars.ts` — define the familiar entry, stats, effect, and cadence
- `src/game/store.ts` — ensure it can be added to the run and persisted
- `src/components/screens/QuestSelect/GearChange.tsx` — expose the familiar slot in loadout management
- `src/components/screens/Battle/Battle.tsx` — hook the automatic turn effect into battle
- `src/components/screens/PostRegionChoice/EventScreen.tsx` — add at least one event reward path for acquisition

### Familiar checklist
- [ ] Add the familiar to `FAMILIAR_DATABASE`
- [ ] Give it a unique `id`, name, icon, rarity, and description
- [ ] Define a single automatic action and an `intervalTurns` cadence
- [ ] Set meaningful stats for HP, AD, AP, armor, MR, speed, and haste
- [ ] Add at least one acquisition source such as an event or battle reward
- [ ] Verify the familiar appears in the team panel and in battle status
