...
The Enemy AI System enables enemies to make intelligent combat decisions while following the same phase-based action economy as the player. Each enemy has a **Behavior Profile** that influences decision-making, but the AI must still respect whether the current timeline action is an attack or spell phase.
...
Enemies do **not** get to freely mix all actions on every timeline event. They follow the same action split as the player:

| Phase | Allowed enemy actions |
|-------|------------------------|
| **Attack / Move phase** | Use a weapon, move, or skip |
| **Spell / Item phase** | Cast a spell, use an item, or skip |

### Phase Rules

1. **Attack and movement share the same phase** and are alternatives, not a combo.
2. **Spells and items share the same phase** and are also alternatives.
3. **Movement is not allowed during spell phases.**
4. **Weapon attacks are not allowed during spell phases.**
5. **If no legal action is good or available, the AI skips.**
...
The AI first checks which phase is active, then evaluates only the actions that are legal for that phase and assigns a **priority score** to each:
...
1. Filter to actions allowed by the current phase
2. Apply **behavior profile modifiers** (see above)
3. Sort actions by score (highest first)
4. Execute the highest-scoring valid action
5. If no valid legal action exists, skip turn
